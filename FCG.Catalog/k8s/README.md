# Kubernetes - CatalogAPI

Este diretório contém os manifestos Kubernetes para executar o microsserviço `catalog-api`.
...
```bash
docker build -t catalog-api:latest -f FCG.Catalog.WebAPI/Dockerfile .
```
...
Você deve ver `catalog-api` com tag `latest` na lista.
...
- `Deployment` `catalog-api-deployment`
- `Service` `catalog-api`
...
O pod do `catalog-api` deve estar com status `Running`.
